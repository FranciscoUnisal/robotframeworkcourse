...
Abra e feche o Notepad
    ...
    ${MEU_PROCESSO}     Start Process    C:\\Program Files\\Google\\Application\\Chrome.exe
    Sleep  3s
    Terminate Process   ${MEU_PROCESSO}   kill=True

Verifica sucesso na execução do processo sucesso
    [Arguments]      ${PROCESSO}
    ...
    Run Keyword If   ${PROCESSO.rc}!=1   Log   FALHA OCORRIDA: "${PROCESSO.stderr}"

Verifica sucesso na execução do processo falha
    [Arguments]      ${PROCESSO}
    ## RC igual a zero indica SUCESSO na execução do processo
    Run Keyword And Continue On Failure    Should Be Equal As Integers    ${PROCESSO.rc}    1
    ...   msg=ATENÇÃO! O seu processo não executou com sucesso
    ## Se ocorreu erro, vamos logar ele
...
Execute um script e aguarde ele finalizar com sucesso
    ...
    Verifica sucesso na execução do processo sucesso      ${MEU_PROCESSO}
...
Execute um script e aguarde ele finalizar com falha
    ...
    Verifica sucesso na execução do processo falha      ${MEU_PROCESSO}
